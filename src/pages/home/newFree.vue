            ...
            <div class="mustList" v-for="(item,index) in list" :key="index">
              <div class="good">
                ...
                <div class="goodMsg">
                  ...
                  <div class="num">
                    剩余<span>{{item.remanent}}</span>单
                  </div>
                </div>
              </div>
              <div class="power">
                <div class="powerMsg"><span v-if="item.rec == '用户等级'">仅限用户等级</span><span v-else>{{item.rec}}</span>: <span class="red">{{item.allow}}</span>  可领取</div>
...
export default {
  ...
  methods: {
    freeGood (){
      this.$http.post('/amoy/home/free-order', {}, true, true).then(res => {
        if(res.code == 0){
