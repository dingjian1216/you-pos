...
export default {
  ...
  data () {
    return {
      tbk: true,
      notUseComponent: ['edit-xf-button', 'edit-goods-slide', 'edit-slide', 'edit-header-category', 'edit-header', 'edit-goods-list','edit-activity-alert'], // 后台传过来这些值时不使用自定义组件component
      times: '',
      goodsList: false,
      slide: true,
      swiperBg: '',
    ...
    upCallback (page, mescroll) {
      ...
      if (this.$store.state.global.content) {
        ...
        if (!this.$store.state.global.goodsList || (this.$store.state.global.goodsList && [0, '0', 'tbk'].indexOf(this.$store.state.global.goodsList.link.type) != -1)) {
          ...
          console.log('好货优选',this.$store.state.global.goodsList)
          // 对后台传过来的"商品列表"组件的数据做处理
          if (this.$store.state.global.goodsList) {
            if (this.$store.state.global.goodsList.link.url == '/select' && this.cid == '') { // 好货优选
              questUrl = '/amoy/opgoods/optional-goods'
              this.info = {page: page.num,cid: this.cid}
            }else if(this.$store.state.global.goodsList.link.url.indexOf("/classify") != -1 && this.cid == ''){
              let classIfy = this.$store.state.global.goodsList.link.url
              for(let i= 0; i<classIfy.length;i++){
                if(classIfy[i] == '_'){
                  console.log(classIfy.substring(i+1))
                  this.info = {
                    page: page.num,
                    type: classIfy.substring(i+1)
                  }
                }
              }
            }
          ...
          this.$http.post(questUrl, this.info, true, true).then((response) => {
            ...
            if(questUrl == '/amoy/opgoods/optional-goods'){
              arr = response.data
            }else{
              arr = response.data.goodsList
            }
